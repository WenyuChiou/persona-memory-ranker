...
data=read_json(root/f'artifacts/selector/presentations/{milestone}-content.json')
build=root/f'artifacts/selector/presentations/{milestone}'
durations=[]
...
out=root/f'deliverables/video/selector/{milestone}-presentation.mp4';out.parent.mkdir(parents=True,exist_ok=True)
...
write_json(root/f'reports/selector/{milestone}_video.json',{'path':out.relative_to(root).as_posix(),'duration_seconds':actual,'target_seconds':target,'voice':'Microsoft David Desktop (synthetic)','tempo_adjustment':tempo,'slides':timeline})
