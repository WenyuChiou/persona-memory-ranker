...
$contentPath = Join-Path $projectRoot "artifacts\presentations\$Milestone-content.json"
$content = Get-Content -LiteralPath $contentPath -Raw -Encoding utf8 | ConvertFrom-Json
$audioDirectory = Join-Path $projectRoot "artifacts\presentations\$Milestone\audio"
