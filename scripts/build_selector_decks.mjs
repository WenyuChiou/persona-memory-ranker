...
const input = JSON.parse(await fs.readFile(path.join(root,`artifacts/selector/presentations/${milestone}-content.json`),'utf8'));
const font = resolvePresentationFont();
const out = path.join(root,'artifacts/selector/presentations',milestone);
...
for (let i=0;i<input.slides.length;i++) {
  ...
  if (info.image) {
    s.images.add({blob:new Uint8Array(await fs.readFile(path.join(root,info.image))),contentType:'image/png',alt:info.title,fit:'contain',position:{left:0,top:0,width:1280,height:720}});
  } else {
  ...
  if (info.chart) {
    ...
    for (const style of [chart.xAxis?.textStyle,chart.yAxis?.textStyle,chart.dataLabels?.textStyle]) if (style) style.fontSize=22;
  } else {
...
const finalPath=path.join(root,`deliverables/selector/${milestone}-presentation${revision ? '-'+revision : ''}.pptx`);
